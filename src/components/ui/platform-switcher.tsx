...
const PlatformSwitcher = React.forwardRef<HTMLButtonElement, PlatformSwitcherProps>(
  (
    {
      icon,
      label,
    ...
    const iconBox = (
      ...
      >
        ...
        {interactive && (
          <ChevronDown className="absolute bottom-0.5 right-0.5 h-3 w-3 opacity-60" />
        )}
      </div>
    )
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent side="right" align="start" className="w-52">
          ...
          {linkedApps &&
            linkedApps.length > 0 &&
            organisations &&
            organisations.length > 0 && <DropdownMenuSeparator />}
