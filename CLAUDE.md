...
6. **Pre-PR verification — sweep before declaring done.** Before marking a component change complete:
   ...
   - **Theme/token sweep:** if you added a new CSS variable in `src/styles/theme.css`, verify both dark and light themes define it; a missing `--color-foo` falls back to `unset` and renders invisibly. Check this in **`dist/styles/index.css` after `npm run build`, not in the source** — Tailwind v4 tree-shakes `@theme` variables nothing references, while the plain `[data-theme='light']` selector below is not pruned. An unreferenced token therefore ships with a light override and NO dark default, which the source file makes look correct. If a token has no consumer yet, don't add it.
